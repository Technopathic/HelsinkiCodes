            ...
            <ContentContainer show={showContent} className="no-scroll-bar bg-opacity-10 bg-whiteBrand dark:bg-darkGrayBrand dark:bg-opacity-10">
                ...
                <div className="relative w-full max-w-screen-lg z-10 flex flex-col mt-28">
                    {props.content.map((item, i) => (
                        <Link href={item.url}>
                            <a index={i} href={item.url} onClick={() => toggleContent(false)} className="bg-whiteBrand rounded-xl w-full p-4 mb-6 no-underline select-none shadow-hcBlueBrand transition-all hover:bg-hcRedBrand dark:bg-darkGrayBrand dark:text-gray-50">
