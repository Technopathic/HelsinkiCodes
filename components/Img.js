...
const Img = (props) => {
    ...
    return (
        <>
            <img src={props.src} alt={props.alt} onClick={() => setShow(true)} className={`cursor-pointer ${props.className}`} />
            <section className={`${show ? 'opacity-100 visible' : 'opacity-0 invisible max-h-0'} flex fixed top-0 bottom-0 left-0 right-0 w-full h-screen overflow-hidden transition-all z-40`}>
                <div className="w-full h-screen bg-opacity-25 bg-black absolute top-0 bottom-0 left-0 right-0 z-40" onClick={() => setShow(false)}></div>
